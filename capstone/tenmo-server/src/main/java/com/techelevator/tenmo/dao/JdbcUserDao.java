...
public class JdbcUserDao implements UserDao {
    ...
    public User getUserById(int userId) {
        ...
        if (results.next()) {
            return mapRowToReturnToUser(results);
        } else {
            return null;
        }
    }
    ...
    @Override
    public List<User> listAllUsersInSystem() {
        List<User> users = new ArrayList<>();
        String sql = "SELECT user_id, username FROM tenmo_user";

        SqlRowSet results = jdbcTemplate.queryForRowSet(sql);
        while (results.next()) {
            User user = mapRowToReturnToUser(results);
            users.add(user);
        }
        return users;
    }
    ...
    @Override
    public List<User> showUsersAvailableForTransfer(int userId) {
        List<User> users = new ArrayList<>();
    ...
    private User mapRowToReturnToUser(SqlRowSet rs) {
        User user = new User();
        user.setId(rs.getInt("user_id"));
        user.setUsername(rs.getString("username"));
//        user.setPassword(rs.getString("password_hash"));
